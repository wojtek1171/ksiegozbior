export default function useLubimyCzytac() {
  ...
  async function getLcData(url: String) {
    ...
    parsedData.value.title = stringData
      ...
      .split('>')[1];
    parsedData.value.authors = stringData
      .match(new RegExp('books:author" content="' + '(.*?)' + '" /><meta property'))[1]
      .split(',&nbsp;');
    parsedData.value.publisher = stringData.match(new RegExp('Wydawnictwo: ' + '(.*?)' + '</'))[1].split('>')[1];
    parsedData.value.category = stringData.match(new RegExp('class="book__category' + '(.*?)' + '</'))[1].split('>')[1];
    ...
    parsedData.value.series = stringData.match(new RegExp('Cykl:' + '(.*?)' + '</'))?.[1]?.split('>')[1];
    parsedData.value.isbn = stringData.match(new RegExp('isbn":"' + '(.*?)' + '"'))[1];
    parsedData.value.tags = stringData.match(/(?<=class="btn btn-outline-primary tag mt-2 mb-0">)(.*?)(?=<\/a>)/g);
    parsedData.value.note = stringData.match(new RegExp('books:rating:value" content="' + '(.*?)' + '" />'))?.[1];
    parsedData.value.description = stringData.match(
      new RegExp('<div class="collapse-content"><p>' + '(.*?)' + '</p>'),
    )[1];
    parsedData.value.image = stringData.match(new RegExp('og:image" content="' + '(.*?)' + '" />'))[1];
    parsedData.value.translators = stringData
      .match(/(?<=Tłumacz:)(.*?)(?=<\/a><\/dd>)/g)
      .map((e) => e.replace(/<.*?>/g, ''));
