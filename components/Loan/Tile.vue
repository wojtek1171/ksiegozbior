...
<template>
  <q-card class="q-mb-xs card-loan" flat bordered>
    <q-card-section class="row wrap q-gutter-sm items-center">
      ...
      <div class="col-12 col-sm-auto col-tab" align="right">
        <q-btn outline icon="archive" color="grey-8" size="md" @click="archiveModalOpen = true">
  ...
  <q-dialog v-model="archiveModalOpen">
    <q-card class="loan-card">
      <q-card-section class="q-pb-none">
        ...
        <div class="text">{{ loan.bookTitle }}</div>
        <div class="text">Pożyczajacy: {{ loan.borrower }}</div>
        <div class="text">Data wypożyczenia: {{ loan.lendingDate.slice(0, 10) }}</div>
      </q-card-section>
      <q-card-section>
        <q-input dense v-model="returnDate" mask="date" :rules="['date']" label="Data oddania">
        ...
        <div class="text">Oddano po {{ getDiffInDays(loan.lendingDate, returnDate) }} dniach.</div>
      </q-card-section>
